...
pub enum ShaderValue {
    ...
    Texture(Entity),
    Buffer(Entity),
    /// Bind a deinterleaved mesh attribute buffer as a read-only storage
    /// binding. First entity is the [`Geometry`](crate::geometry::Geometry);
    /// second is the [`Attribute`](crate::geometry::Attribute) selecting the
    /// vertex binding.
    MeshAttribute(Entity, Entity),
    /// Bind a mesh's index buffer as a read-only storage binding.
    MeshIndex(Entity),
}
